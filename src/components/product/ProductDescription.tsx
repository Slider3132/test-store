...
export function ProductDescription({ product }: { product: Product }) {
  ...
  if (hasVariants) {
    ...
    const firstCompareAtPrice = variantsOrderedByPrice.find(
      (variant) => typeof variant.compareAtPrice === 'number',
    )?.compareAtPrice
    if (
    ...
    if (typeof firstCompareAtPrice === 'number') {
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="uppercase font-mono">
          {compareAtPrice ? (
            <Price
              amount={compareAtPrice * 100}
              as="span"
              className="mr-3 text-sm text-muted-foreground line-through"
            />
          ) : null}
          {hasVariants ? (
