...
import { getSelectedOrDefaultVariant, getVariantSearchParams } from './variantSelection'
...
export function VariantSelector({ product }: { product: Product }) {
  ...
  const selectedOrDefaultVariant = getSelectedOrDefaultVariant(product, searchParams)
  const selectedOrDefaultParams = getVariantSearchParams(product, selectedOrDefaultVariant)
  ...
  return variantTypes?.map((type) => {
    ...
    return (
      <dl className="" key={type.id}>
        <dt className="mb-4 text-sm">{localizeVariantLabel(type.label, dictionary.product)}</dt>
          ...
          <React.Fragment>
            {options?.map((option) => {
              ...
              const selectedOptionID =
                searchParams.get(optionKeyLowerCase) || selectedOrDefaultParams.get(optionKeyLowerCase)
              const isActive = Boolean(isAvailableForSale) && selectedOptionID === String(optionID)
              ...
              return (
                <Button
                  ...
                  title={`${localizeVariantLabel(option.label, dictionary.product)} ${!isAvailableForSale ? ` (${dictionary.product.outOfStock})` : ''}`}
                >
                  {localizeVariantLabel(option.label, dictionary.product)}
                </Button>
              )
            })}
          </React.Fragment>
