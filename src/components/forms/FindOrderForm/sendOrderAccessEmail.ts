...
export async function sendOrderAccessEmail({
  ...
  try {
    ...
    const orderToken = createOrderAccessToken({ email, orderID: String(order.id) })
    const orderURL = `${serverURL}/orders/${order.id}?email=${encodeURIComponent(email)}&orderToken=${encodeURIComponent(orderToken)}`
