  ...
  return (
    <Sheet onOpenChange={setIsOpen} open={isOpen}>
      ...
      <SheetContent className="flex flex-col">
        <SheetHeader>
          <SheetTitle>{dictionary.cart.title}</SheetTitle>

          <SheetDescription>{dictionary.cart.description}</SheetDescription>
        </SheetHeader>

        {!cart || cart?.items?.length === 0 ? (
          <div className="text-center flex flex-col items-center gap-2">
            <ShoppingCart className="h-16" />
            <p className="text-center text-2xl font-bold">{dictionary.cart.empty}</p>
          </div>
        ) : (
            ...
            <div className="flex flex-col justify-between w-full">
              <ul className="grow overflow-auto py-4">
                {cart?.items?.map((item, i) => {
                  ...
                  if (isVariant) {
                    ...
                    if (!variant.image && imageVariant && typeof imageVariant.image === 'object') {
              ...
              <div className="px-4">
                <div className="py-4 text-sm text-neutral-500 dark:text-neutral-400">
                  ...
                  <Button asChild>
                    <Link className="w-full" href={localizePath('/checkout', locale)}>
